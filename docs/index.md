...
There are two main classes that you might want to use depend on the store used in a vue application (currently only pinia is implemented)

- [PiniaOidcStore](classes/PiniaOidcStore.html) provides a higher level API for
  signing a user in, signing out, managing the user's claims
- [VuexOidcStore](classes/VuexOidcStore.html) provides the raw OIDC/OAuth2 protocol
  support
