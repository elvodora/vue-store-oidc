...
describe("createStoreModule", function () {
  before(function () {
    OidcStore = require("../dist/pinia-oidc.cjs.js");
    storeModule = OidcStore.piniaOidcCreateStoreModule(oidcConfig, {
  ...
  it("factory function should return a pinia module object", function () {
  ...
  describe(".actions.oidcCheckAccess", function () {
    describe("check public routes", function () {
      it("should resolve true for public routes if authenticated", function () {
        ...
        return storeModule.actions
          .oidcCheckAccess(context, publicRoute())
    ...
    describe("check callback routes", function () {
      it("should resolve true for oidcCallbackRoutes routes if authenticated", function () {
        return storeModule.actions
          .oidcCheckAccess(authenticatedContext(), oidcCallbackRoute())
          .then(function (hasAccess) {
            assert.equal(hasAccess, true);
          });
      });
      it("should resolve true for oidcCallbackRoutes routes if not authenticated", function () {
        return storeModule.actions
          .oidcCheckAccess(unAuthenticatedContext(), oidcCallbackRoute())
          .then(function (hasAccess) {
            assert.equal(hasAccess, true);
          });
      });
    ...
    describe("check silent callback routes", function () {
      it("should resolve true for oidcSilentCallbackRoutes routes if authenticated", function () {
        return storeModule.actions
          .oidcCheckAccess(authenticatedContext(), oidcSilentCallbackRoute())
          .then(function (hasAccess) {
            assert.equal(hasAccess, true);
          });
      });
      it("should resolve true for oidcSilentCallbackRoutes routes if not authenticated", function () {
        return storeModule.actions
          .oidcCheckAccess(unAuthenticatedContext(), oidcSilentCallbackRoute())
          .then(function (hasAccess) {
            assert.equal(hasAccess, true);
          });
      });
    ...
    describe("check protected routes", function () {
      it("should resolve false for protected routes if not authenticated, and also dispatch auth redirect action", function () {
        ...
        sinonSandbox
          .stub(UserManager.prototype, "signinSilent")
          .callsFake(silentSigningFailedPromise);
        sinonSandbox
          .stub(UserManager.prototype, "getUser")
          .callsFake(getNoUserPromise);
        return storeModule.actions
          .oidcCheckAccess(context, protectedRoute())
          .then(function (hasAccess) {
      ...
      it("should resolve true for protected routes if authenticated in both pinia and in storage", function () {
        const context = authenticatedContext();
        sinonSandbox
          .stub(UserManager.prototype, "getUser")
          .callsFake(getUserPromise);
        return storeModule.actions
          .oidcCheckAccess(context, protectedRoute())
          .then(function (hasAccess) {
            assert.equal(hasAccess, true);
      ...
      it("should resolve true for protected routes if not authenticated in pinia, but in storage.", function () {
        const context = unAuthenticatedContext();
        sinonSandbox
          .stub(UserManager.prototype, "getUser")
          .callsFake(getUserPromise);
      ...
      it("should resolve false for protected routes if authenticated in pinia, but not in storage. Also signout user from pinia and dispatch auth redirect action.", function () {
        const context = authenticatedContext();
        sinon.spy(context, "dispatch");
  ...
  describe(".actions.authenticateOidc", function () {
    it("performs signinRedirect with empty arguments by default", function () {
      ...
      sinonSandbox
        .stub(UserManager.prototype, "signinRedirect")
        .callsFake(resolveArgumentsPromise);
  ...
  describe(".getters.oidcIsRoutePublic", function () {
    it("should not call isPublicRoute when not a function", function () {
      ...
      storeModule = OidcStore.piniaOidcCreateStoreModule(oidcConfig, {
        isPublicRoute: "not a function",
      });
      assert.equal(
        storeModule.getters.oidcIsRoutePublic(storeModule.state)(route),
        false
      );
    });
...
function authenticatedContext(storeSettings = {}) {
  const context = Object.assign(
    OidcStore.piniaOidcCreateStoreModule(oidcConfig, storeSettings, {
      oidcError: oidcMockOidcError,
    }),
    {
